...
- 色は手順5/7で補正済みの表示/backfill 用画像をモデル解像度に縮小した基準色を、割り当て
  済みラベルに沿って伝播し、同一ラベル内で反復平滑化して延長方向の縞を緩和する
- 主メッシュには一切手を加えない
- backfill メッシュ生成（js/viewer.js）では、種の手前突出を 1e で除去済みのため背景同士は
  極力面で繋ぎ（視差時の黒穴を防ぐ）、極端な段差（近深度の `BACKFILL_DEPTH_JUMP`=1.0 倍超、
  例: 背景 vs 空バックドロップ）だけ除去する。値を上げるほど黒穴は減り、離れた背景を繋ぐ薄壁が増える
