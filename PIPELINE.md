...
- 走査検出（追加、TASK_G G-6）: 毛・草などソフトな輪郭の重なりで出る「数十px のなだらかな
  ランプ」（1画素 1〜2% で窓検出にも掛からない）を、行/列走査の run で検出する。同符号の
  disparity 勾配が連続する区間が「長さ `48px` 以下 かつ 総 disparity 変化 > `0.25` × シーン
  中央値 disparity」なら全画素を marked にする（step 下限 `0.003`×中央値/px）。しきい値は
  **視差（parallax）基準**: 切り離すべきかは深度比ではなく視点移動時の画面上のズレ量
  ∝ disparity 差で決まるため、遠景は深度比が2倍でも拾わず、近景は小さな深度比でも拾う
  （backfill メッシュの視差カットと同じスケール不変の考え方）。本物の斜面（床・壁）は
  同じ勾配が何百px も続いて run が長くなるため除外される。さらに**両端条件**: 膜は両端が
  台地（勾配ほぼ0）で終わるのに対し、布の折り目・接地際・尾根は符号反転で終わる
  （下り→谷底→上り）ため、両端とも台地かデータ境界で終わる run だけを marked にする。
  連続面の折り目に穴を開けず、細い毛・枝も保護される
...
- 上限を超えて残った marked pixel（未解決の中間深度）は**無効化**する。中間深度のまま
  頂点になるとシルエットで前後スパイク（櫛状の膜）になるため、面を張らず backfill に
  裏打ちさせる（三重点セル skip と同じ思想。切替定数 `INVALIDATE_UNRESOLVED`、
  docs/tasks/TASK_G_EDGE_SPIKE.md G-1）
- 変更するもの: depth と points（レイ上の前後移動のみ）、未解決画素の有効/無効
- 変更しないもの: UV・テクスチャ
